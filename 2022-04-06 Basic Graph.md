...
public class BasicGraph<V> { 
    // V so we can index our vertices according to String/int
   
    // Keep an index from node labels to nodes in the map
    protected Map<V, Vertex<V>> vertices; //Hash map
    ...
    public void addVertex(V u) { // if only provided with name
        addVertex(new Vertex<>(u));
    }
    
    public void addVertex(Vertex<V> v) { // if provided with vertex object
    ...
    protected class Vertex<V> {
        public V name; // ie. V1, V2
        private List<Edge<V>> adjacent; //List of adjacencies
        int indegree; 
    ...
    public void computeIndegrees() {
      ...
      for (Vertex<V>  v: vertices.values()) { 
        v.indegree = 0; // to reset before recalculating indegree
     }
    ...
    public List<V> topo_sort() {
      ...
      for (Vertex<V> v : vertices.values()) // find all vertices with 0 indegree and enqueue; initializing
        if (v.indegree == 0)
          queue.addFirst(v); //addLast for queue, addFirst for stack

      List<V> result = new LinkedList<>();

      while (queue.size() > 0) {  // main loop, repeat until queue is empty 
        
        Vertex<V> u = queue.pollFirst(); //grabs first vertex; visit
        result.add(u.name);
    ...
    public void bfs_shortest_path(V start) { //start vertex

      for (Vertex<V> v: vertices.values()) { // initializing costs and prev markers
