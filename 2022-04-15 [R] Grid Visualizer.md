...
public class GridVisualizer extends Canvas {
    // allows us to visualize in 2D space
  
  static int rows = 10;
  static int columns = 18;
  static int cell_size = 80; // in pixels for visualizer
  boolean[][] obstacles; //represents where obstacles are located
  boolean[][] discovered;
  boolean[][] visited; 
  static int startRow = 0; //location of start square
  static int startColumn = 0;  
  static int destRow = 7; //location of target square
  ...
  private static class Cell implements Comparable<Cell>{
    ...
    public double distanceToGoal() {
      return Math.sqrt(Math.pow(row - destRow, 2) + Math.pow(column - destColumn, 
2)); // distance formula
    }
  } 
  // transition function (instead of keeping adjacency list)
    // save memory, don't need to store all edges
  public List<Cell> getAdjacentCells(Cell cell) {
   
    List<Cell> result = new LinkedList<>();
 
    for (int i = 1; i >= -1; i--) // can go diagonally
      for (int j = 1; j >= -1; j--) 
        if (! (i==0 && j==0)) {
          int newRow = cell.row + i;
          int newColumn = cell.column + j;
          if ((newRow >= 0 && newColumn >= 0 && newRow < rows && newColumn < 
columns) && 
  ...
  public void runBfs() throws InterruptedException {
    ...
    queue.addLast(new Cell(startRow, startColumn)); //adds starting position
    ...
    repaint(); // provided by Canvas class, calls paint() to redraw
    Thread.sleep(100); // to give the current grid time to display
    }           
  }
  ...
  public void runBestFirst() throws InterruptedException {
   //will visit cell with shortest distance to goal first
    PriorityQueue<Cell> queue = new PriorityQueue<>();
      //using priorityqueue means we need the Cell class to be Comparable
...
![image-20220427200338161](images/image-20220427200338161.png)

Find shortest path from green square to red square

- Can move up, down, left, right, and diagonally
- Can’t move through black squares



Represent Grid with rows/columns:

<img src="images/image-20220427201219556.png" alt="image-20220427201219556" style="zoom:50%;" />

- Keep an adjacency list of possible squares that can be moved to
  - Each square coordinate is a vertex
- An undirected graph since square can move in both directions



If exploring the graph in a BFS:

- Adjacent squares of visited square will be added to the agenda
- Visit next square on agenda and add its adjacent squares to agenda
- Continues until red square is reached



If exploring the graph in a DFS:

- Will visit square in different order since stack
- Only when we reach a corner will we “backtrack”